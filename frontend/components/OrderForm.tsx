  ...
  if (!isConnected) {
    return (
      <div className="card p-10 flex flex-col items-center justify-center min-h-[280px] gap-5">
        <div className="w-14 h-14 rounded-2xl bg-[var(--bg-elevated)] flex items-center justify-center float">
    ...
    <div className="card overflow-hidden">
      ...
      <div className="flex border-b border-[var(--border-subtle)]">
        {[
          ...
          <button
            key={s}
            onClick={() => setSide(s)}
            className={`flex-1 py-4 text-[14px] font-semibold transition-all duration-300 relative cursor-pointer ${
              side === s
                ? s === SIDE_BUY
          ...
          >
            {label}
            {side === s && (
              <div className={`absolute bottom-0 left-0 right-0 h-[2px] tab-underline ${
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl text-[14px] font-semibold btn-lift cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 ${
            side === SIDE_BUY
      ...
      {/* Status */}
      {status && (
        <div className={`mx-6 mb-6 px-4 py-3 rounded-xl text-[13px] font-mono break-all border slide-in-right ${
          txSuccess
