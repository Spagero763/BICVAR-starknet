...
const WALLET_INFO = {
  argentX: {
    ...
    deepLink: (url: string) => `argent://app/browser?url=${encodeURIComponent(url)}`,
  },
  braavos: {
    name: "Braavos",
    color: "var(--cyan)",
    ...
    deepLink: (url: string) => `braavos://browser?url=${encodeURIComponent(url)}`,
  },
};
...
function isInsideWalletBrowser(): boolean {
  ...
  return !!(w.starknet || w.starknet_argentX || w.starknet_braavos);
}

function getDappUrl(): string {
  if (typeof window === "undefined") return "https://bicvar-starknet.vercel.app";
...
function WalletNotFoundModal({ walletKey, onClose }: { walletKey: string | null; onClose: () => void }) {
  ...
  const isDeepLinkMode = walletKey === "deeplink" || (isMobile && !inWalletBrowser);
      ...
      <div className="relative w-full max-w-sm border border-[var(--border-default)] bg-[var(--bg-card)] shadow-2xl shadow-black/60 modal-in overflow-y-auto" style={{ maxHeight: '80dvh' }}>
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-mono font-bold text-[var(--text-primary)] tracking-[0.15em] uppercase">
              {isDeepLinkMode ? "Open in Wallet" : "Wallet Not Detected"}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {isDeepLinkMode ? (
            <>
              <p className="text-[11px] font-mono text-[var(--text-secondary)] leading-relaxed">
                Mobile browsers can&apos;t run wallet extensions. Open this page inside your wallet&apos;s built-in browser to connect.
              </p>

              {/* Deep link buttons */}
              <p className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.2em] uppercase">// Already have a wallet?</p>
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => handleDeepLink(WALLET_INFO.argentX.deepLink(dappUrl))}
                  className="flex items-center justify-between px-3 py-3 border border-[var(--accent)]/30 hover:border-[var(--accent)]/60 bg-[var(--accent-light)] transition-all group cursor-pointer"
                >
                  <div className="flex items-center gap-2.5">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M15 3h6v6" /><path d="M10 14L21 3" /><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                    </svg>
                    <span className="text-[12px] font-mono font-bold text-[var(--accent)] tracking-wider">Open in Argent</span>
                  </div>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6" /></svg>
                </button>
                <button
                  onClick={() => handleDeepLink(WALLET_INFO.braavos.deepLink(dappUrl))}
                  className="flex items-center justify-between px-3 py-3 border border-[var(--cyan)]/30 hover:border-[var(--cyan)]/60 bg-[var(--bg-primary)] transition-all group cursor-pointer"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-[12px] font-mono font-bold text-[var(--cyan)] tracking-wider">Open in Braavos</span>
                  </div>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--cyan)" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6" /></svg>
                </button>
              </div>

              {/* Download section */}
              <div className="border-t border-[var(--border-subtle)] pt-3">
                <p className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.2em] uppercase">// Don&apos;t have a wallet yet?</p>
                <div className="flex gap-2 mt-2">
                  <a href={wallet.ios} target="_blank" rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 border border-[var(--border-subtle)] hover:border-[var(--text-muted)]/30 bg-[var(--bg-primary)] transition-all cursor-pointer group">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] transition-colors">
                      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                    </svg>
                    <span className="text-[9px] font-mono font-bold text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] tracking-wider uppercase transition-colors">App Store</span>
                  </a>
                  <a href={wallet.android} target="_blank" rel="noopener noreferrer"
                    ...
                    <span className="text-[9px] font-mono font-bold text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] tracking-wider uppercase transition-colors">Google Play</span>
                  </a>
                </div>
              </div>
              ...
              <div className="border-l-2 border-[var(--accent)]/20 bg-[var(--accent-light)] px-3 py-2">
                <p className="text-[9px] font-mono text-[var(--text-secondary)] leading-relaxed">
                  <span className="text-[var(--accent)] font-bold">HOW IT WORKS: </span>
                  Tap &quot;Open in Argent/Braavos&quot; above &rarr; The wallet app opens this page in its built-in browser &rarr; Connect automatically
                </p>
              </div>
            </>
          ) : (
